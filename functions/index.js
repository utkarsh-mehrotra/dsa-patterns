 ...
 * Uses a double-redundant serverless architecture: Judge0 CE -> Piston custom fallback
 */
app.post("/api/execute", async (req, res) => {
  try {
    ...
    // Mode 2: Attempt execution via custom Piston API
    try {
      let pistonLang = "";
      let version = "";
      let fileName = "";
    ...
    return res.status(500).json({
      error: "An internal cloud execution error occurred while executing code.",
      details: error.message || error
    });
  }
});
